...
bool    ImGui::BeginTable(const char* str_logo, const char* str_id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ImGuiID id = GetID(str_id);
    return BeginTableEx(str_logo,str_id, id, columns_count, flags, outer_size, inner_width);
}

bool    ImGui::BeginTableEx(const char* logo, const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ImGuiContext& g = *GImGui;
    ...
    if (use_child_window)
    {
        ...
        BeginChildEx(logo,name, instance_id, outer_rect.GetSize(), false, child_flags);
